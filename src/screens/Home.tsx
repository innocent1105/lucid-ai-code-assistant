...
export default function GeminiLucid() {
  const API_BASE_URL = "http://10.243.184.234/lucid/lucid-ai/backend";
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try{
        ...
        await db.messages.add({
                conversationId: convId!,
                role: 'assistant',
                content: `I cant process your request without internet connection. Please connect and try again.`,
                timestamp: Date.now(),
            })
            setIsLoading(false);
    }
